...
function EU:OnInitialize()
   ...
   self.Log = Log:New "EU"
	...
	self.optionsColOrder = addon.isClassic
                       					and {
  ...
  "votes", "vote", "note", "roll"
		...
		"class", "name", "rank", "role", "response", "ilvl", "diff", "gear1", "gear2",
  "votes", "vote", "note", "roll"
	}
...
function EU:OnEnable()
   ...
   self:SetupComms()
	...
	self:Hook(self.votingFrame, "SwitchSession", function(_, s)
		session = s
	end)
...
function EU:HandleExternalRequirements()
	...
	-- RCScore
	if self.db.columns.rcscore and self.db.columns.rcscore.enabled
					and not (Details or Recount or Skada) then
	...
	for _, v in ipairs(cols) do
		...
		if self.db.columns[v.name] then -- handle EU column
			...
			tinsert(newCols, {
				...
				colName = v.name,
				sortNext = temp.sortNext or self:GetScrollColIndexFromName("reponse")
			})
	...
	for index in ipairs(self.votingFrame.scrollCols) do
		if self.votingFrame.scrollCols[index].sortNext then
			local exists = self:GetScrollColIndexFromName(
               							self.sortNext[self.votingFrame.scrollCols[index].colName])
...
function EU:UpdateColumnPosition(name, pos)
	...
	if pos > #self.votingFrame.scrollCols then pos = #self.votingFrame.scrollCols end
   ...
   EU.Log:D("calcPawnScore for ", spec)
	...
	if PawnVersion then
		for session, v in ipairs(lootTable) do
			score[session] = {}
			score[session].new = addon.round(EU:GetPawnScore(v.link, addon.playerClass,
                                                 			spec) or 0, 3)
	...
	for i = 1, 17 do
		if i ~= 4 then
			...
			if link then
				...
				local color, itemType, itemID, enchantID, gemID1, gemID2, gemID3, gemID4,
										suffixID, uniqueID, linkLevel, specializationID, upgradeTypeID,
										upgradeID, instanceDifficultyID, numBonuses, bonusIDs =
				...
				if numBonuses > 0 then
					for _, v in ipairs(bonusIDs) do
						if forgedTable[v] then titanforged = titanforged + 1 end
					end
				end

				if (select(16, GetItemInfo(link))) then setPieces = setPieces + 1 end
			end
		end
	end
...
function EU.SetCellPawn(rowFrame, frame, data, cols, row, realrow, column,
	...
	if EU.db.acceptPawn and playerData[name] and playerData[name].pawn
					and playerData[name].pawn[session] and playerData[name].pawn[session].new then
		-- For this we rely on our own storage:
		if EU.db.pawnNormalMode then
			score = playerData[name].pawn[session].new
		elseif playerData[name].pawn[session].equipped > 0
						and playerData[name].pawn[session].new > 0 then
			score = (playerData[name].pawn[session].new
        							/ playerData[name].pawn[session].equipped - 1) * 100
		elseif playerData[name].pawn[session].equipped == 0
						and playerData[name].pawn[session].new > 0 then -- Major, not realistic, upgrade
			score = 100
		end

		-- If we've already calculated it, then just retrieve it from the votingFrame data:
	elseif playerData[name] and playerData[name].pawn
					and playerData[name].pawn[session] and playerData[name].pawn[session].own then
	...
	if playerData[name] and playerData[name].bonusType then
		...
		if type == "item" or type == "artifact_power" then
			...
			f:SetScript("OnClick", function()
				if IsModifiedClick() then HandleModifiedItemClick(link); end
			end)
...
function EU.SetCellRCScore(rowFrame, frame, data, cols, row, realrow, column,
	...
	if ilvl and ilvl ~= "" then
		...
		frame.text:SetTextColor(unpack(colorGradient[math.ceil(score / 10)]
                               						or {0, 1, 0})) -- >100% is not included in the table, just make it green
	else -- Clear it
		frame.text:SetText("")
		data[realrow].cols[column].value = 0
	end
